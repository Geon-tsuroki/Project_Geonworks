...
function fixedHeader() { // 210804 서정환 수정
	...
	if(scrollY > header.offsetTop) {
		...
		//fixed_margin.style.marginTop  = header_height; 0429 수정 - 사유 : 스크롤 시 상단 margin-top 발생으로 인한 덜컹거림
    } else {
...
function top_category(){
	...
	/* =========================================================
	   [추가된 코드] PC 카테고리 클릭 시 전체 리스트 이동 방지
	   ========================================================= */
	// .off('click')을 추가한 이유: 
	// observeTopCategory(변경 감지)에 의해 이 함수가 여러 번 실행될 수 있으므로, 
	// 이벤트가 중복으로 쌓이는 것을 방지하기 위함입니다.
	jQuery('#header .top_category li > a').off('click').on('click', function(e) {
		var hasSubMenu = jQuery(this).siblings('ul').length > 0;
		if (hasSubMenu) {
			e.preventDefault(); // 하위 카테고리(ul)가 존재하면 링크 이동 막기
		}
	});
}
